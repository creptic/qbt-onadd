...
function change_max_upload () {
  ...
  if [ "$is_num" != "" ]; then is_invalid="(Invalid KB/s)" ; fi 
     if [[ "$t_maxup" != "" && "$dry_run" != "1" ]]; then
        if [ "$is_num" == "" ]; then 
           ...
           dg.print "  -Max upload speed Invalid not set:[""$t_maxup""]" "$is_invalid"
        fi
     else
     if [[ "$dry_run" == "1" && "$t_maxup" != "" ]]; then 
        dg.print "  -Max download speed not set:[""$t_maxup""]"  "$is_invalid" 
     fi
  fi 
}
  ...
  if [ "$is_num" != "" ]; then is_invalid="(Invalid KB/s)" ; fi 
     if [[ "$t_maxdl" != "" && "$dry_run" != "1" ]]; then
        if [ "$is_num" == "" ]; then 
           ...
           dg.print "  -Max download speed Invalid not set:[""$t_maxdl""]"  "$is_invalid"
        fi
     else
     if [[ "$dry_run" == "1" && "$t_maxdl" != "" ]]; then 
        dg.print "  -Max download speed not set:[""$t_maxdl""]"  "$is_invalid" 
     fi
  fi 
}
...
function change_category () {   
 #Change category name if changed (check if exists, if not add new category and set)
  local t_new_category="$1"
  if [[ "$t_new_category" != "" && "$dry_run" != "1" ]]; then
     local is_cat=`($qbt_cli category list -F csv | tr -s "\n" "," | cut -c15-)`
     if [ "$t_new_category" != "$category" ]; then    
        local t_found=""
...
function check_connection {
  ...
  local t_con=`$qbt_cli global info -F property:name="connection status"`
  if [ "$t_con" != "" ]; then 
     g.print "  -Checking connection:[""$t_con""]" 
  else
...
if [ "$category_found" == "1" ]; then 
   if [ "$check_trackers_if_category_found" != "1" ]; then
      ...
      if [ "$check_trackers_if_category_found" == "1" ]; then 
         ...
         tag="" # If tag was set, then unset name to possibly reuse 
